...
test('buildEntry records headers + body for a non-2xx response, redacting secrets', () => {
  ...
  const entry = buildEntry(fakeReq(), res, 1, 'test-app', '2026-07-06T00:00:00.000Z', '{"error":"boom"}');
...
test('buildEntry never records headers/body for a 2xx response even if a body is passed', () => {
  const entry = buildEntry(fakeReq(), fakeRes({ statusCode: 204 }), 1, 'test-app', '2026-07-06T00:00:00.000Z', 'should be ignored', true);
...
test('buildAppLogEntry formats a message and keeps structured params', () => {
  const entry = buildAppLogEntry('info', ['Fetching %s (attempt %d)', 'http://x/y', 2], 'test-app', '2026-07-06T00:00:00.000Z');
  assert.deepEqual(entry, {
    ts: '2026-07-06T00:00:00.000Z',
    app: 'test-app',
    level: 'info',
