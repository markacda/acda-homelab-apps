...
function fetchPfData() {
  if (fetchingPf) return;
  fetchingPf = true;
  for (let i in pf_data) {
    const req = jQuery.ajax({ url: pf_data[i], dataType: 'json' });
    jQuery.when(req).done(function (data) {
      for (let i in g.planesOrdered) {
        ...
        if (!plane.registration && ac.reg && ac.reg != '????' && ac.reg != 'z.NO-REG') plane.registration = ac.reg;
...
function zoomIn() {
  ...
  if (FollowSelected) toggleFollow(true);
}
...
function labelZoomScaleFor(zoom) {
  if (zoom >= LABEL_FULL_ZOOM) return 1;
...
function changeZoom(init) {
  ...
  if (!init && Math.abs(g.zoomLvl - g.zoomLvlCache) < 0.4) return;
  ...
  if (!init && (g.zoomLvl < LABEL_FULL_ZOOM || prevZoom < LABEL_FULL_ZOOM)) refreshFeatures();

  if (!init && showTrace) updateAddressBar();
...
function changeCenter(init) {
  ...
  if (CenterLat < -85) OLMap.getView().setCenter(ol.proj.fromLonLat([center[0], -85]));
  if (CenterLat > 85) OLMap.getView().setCenter(ol.proj.fromLonLat([center[0], 85]));
}
...
function checkMovement() {
  ...
  if (checkMoveZoom != zoom || checkMoveCenter[0] != center[0] || checkMoveCenter[1] != center[1]) {
    checkMoveDone = 0;
    if (FollowSelected) {
      checkFollow();
    }
  ...
  if (!checkMoveDone && heatmap && elapsed > 300) {
    if (!heatmap.manualRedraw) drawHeatmap();
    checkMoveDone = 1;
  }
...
function mapRefresh(redraw) {
  if (!mapIsVisible || heatmap) return;
  let addToMap = [];
  let nMapPlanes = 0;
  let count = 0;
...
function highlight(evt) {
  ...
  if (HighlightedPlane && hex == HighlightedPlane.icao) return;
...
function processURLParams() {
  ...
  if (urlIcaos.length > 0) {
    ...
    if (!usp.has('noIsolation') && !usp.has('replay')) toggleIsolation('on');
    if (icaos.length > 1) {
  ...
  if (usp.has('leg')) {
    ...
    if (isNaN(legSel) || legSel < -1) legSel = -1;
    else legSel--;
  }

  let tracks = usp.get('monochromeTracks');
  if (tracks != undefined) {
    if (tracks.length == 6) monochromeTracks = '#' + tracks;
    else monochromeTracks = '#000000';
  }

  let markers = usp.get('monochromeMarkers');
  if (markers != undefined) {
    if (markers.length == 6) monochromeMarkers = '#' + markers;
    else monochromeMarkers = '#FFFFFF';
  }

  let outlineColor = usp.get('outlineColor');
  if (outlineColor != undefined) {
    if (outlineColor.length == 6) OutlineADSBColor = '#' + outlineColor;
    else OutlineADSBColor = '#000000';
  }
...
function regIcaoDownload(opts) {
  ...
  let req = jQuery.ajax({
    url: 'api/globe-airplanes-live/' + databaseFolder + '/regIcao.js',
    cache: true,
    timeout: 60000,
    dataType: 'json',
    opts: opts,
...
function findPlanes(queries, byIcao, byCallsign, byReg, byType, showWarnings) {
  ...
  if (queries.length > 1) toggleMultiSelect('on');
  let results = [];
...
function globe_index(lat, lon) {
  ...
  for (let i = 0; i < globeIndexSpecialTiles.length; i++) {
    ...
    if (
      lat >= tile[0] &&
      lat < tile[2] &&
      ((tile[1] < tile[3] && lon >= tile[1] && lon < tile[3]) || (tile[1] > tile[3] && (lon >= tile[1] || lon < tile[3])))
    ) {
...
function inView(pos, ex) {
  ...
  if (lat < ex.minLat || lat > ex.maxLat) return false;
...
function updateAddressBar() {
  ...
  if (heatmap || (pTracks && !haveTraces) || !CenterLat || uuid) return;
  ...
  if (SelPlanes.length > 0 && showTrace) {
    ...
    if (legSel != -1) string += '&leg=' + (legSel + 1);
...
function refreshInt() {
  ...
  if (inactive < base) inactive = base;
  if (inactive > 4 * base) inactive = 4 * base;

  if (globeIndex) {
    refresh *= inactive / base;
  }

  if (!mapIsVisible) refresh *= 2;
  ...
  if (document.visibilityState === 'hidden') {
    refresh *= 4;
  } // in case visibility change events don't work, reduce refresh rate if visibilityState works
