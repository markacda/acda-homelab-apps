...
(function (global, jQuery, TAR) {
  ...
  function compareBeta(xa, ya) {
    if (xa === ya) return 0;
    if (planeMan.sortAscending && xa < ya) return -1;
    if (!planeMan.sortAscending && xa.replace(/ /g, '').split('').reverse().join('') > ya.replace(/ /g, '').split('').reverse().join('')) return -1;
    return 1;
  }
  ...
  cols.country = {
    ...
    value: function (plane) {
      return plane.country_code
        ? '<img width="18" height="12" style="display: block;margin: auto;" src="flags/3x2/' +
            plane.country_code.toUpperCase() +
            '.svg" title="' +
            plane.country +
            '"></img>'
        : '';
  ...
  cols.flight = {
    ...
    value: function (plane) {
      if (flightawareLinks) return getFlightAwareModeSLink(plane.icao, plane.flight, plane.name);
      return plane.flight || '';
    },
  ...
  if (routeApiUrl) {
    cols.route = {
      sort: function () {
        sortBy('route', compareAlpha, function (x) {
          return x.routeColumn;
        });
      },
      value: function (plane) {
  ...
  cols.registration = {
    ...
    value: function (plane) {
      return flightawareLinks ? getFlightAwareIdentLink(plane.registration, plane.registration) : plane.registration ? plane.registration : '';
    },
    html: flightawareLinks,
    text: 'Registration',
  };
  cols.type = {
    sort: function () {
      sortBy('type', compareAlpha, function (x) {
        return x.icaoType;
      });
    },
    value: function (plane) {
      return plane.icaoType != null ? plane.icaoType : '';
    },
    text: 'Type',
  };
  cols.squawk = {
    ...
    value: function (plane) {
      return plane.squawk != null ? plane.squawk : '';
    },
    align: 'right',
  };
  cols.altitude = {
    ...
    sort: function () {
      sortBy('altitude', compareNumeric, function (x) {
        return x.altitude == 'ground' ? -100000 : x.altitude;
      });
    },
    value: function (plane) {
      return format_altitude_brief(adjust_baro_alt(plane.altitude), plane.vert_rate, DisplayUnits);
    },
    align: 'right',
    header: function () {
      return 'Alt.' + NBSP + '(' + get_unit_label('altitude', DisplayUnits) + ')';
    },
  };
  cols.speed = {
    ...
    value: function (plane) {
      return format_speed_brief(plane.speed, DisplayUnits);
    },
    align: 'right',
    header: function () {
      return (pTracks ? 'Max. ' : '') + 'Spd.' + NBSP + '(' + get_unit_label('speed', DisplayUnits) + ')';
    },
  };
  cols.vert_rate = {
    text: 'Vertical Rate',
    ...
    value: function (plane) {
      return format_vert_rate_brief(plane.vert_rate, DisplayUnits);
    },
    align: 'right',
    header: function () {
      return 'V. Rate(' + get_unit_label('verticalRate', DisplayUnits) + ')';
    },
  };
  cols.sitedist = {
    ...
    value: function (plane) {
      return format_distance_brief(plane.sitedist, DisplayUnits);
    },
    align: 'right',
    header: function () {
      return (pTracks ? 'Max. ' : '') + 'Dist.' + NBSP + '(' + get_unit_label('distance', DisplayUnits) + ')';
    },
  };
  cols.track = {
    text: 'Track',
    ...
    value: function (plane) {
      return format_track_brief(plane.track);
    },
    align: 'right',
  };
  cols.msgs = {
  ...
  cols.rssi = {
    ...
    value: function (plane) {
      return plane.rssi != null ? plane.rssi.toFixed(1) : '';
    },
    align: 'right',
  };
  cols.lat = {
    ...
    value: function (plane) {
      return plane.position != null ? plane.position[1].toFixed(4) : '';
    },
    align: 'right',
  };
  cols.lon = {
    ...
    value: function (plane) {
      return plane.position != null ? plane.position[0].toFixed(4) : '';
    },
    align: 'right',
  };
  cols.data_source = {
    text: 'Source',
    sort: function () {
      sortBy('data_source', compareNumeric, function (x) {
        return x.getDataSourceNumber();
      });
    },
    value: function (plane) {
  ...
  cols.military = {
    text: 'Mil.',
    sort: function () {
      sortBy('military', compareAlpha, function (x) {
        return x.military ? 'yes' : 'no';
      });
    },
    value: function (plane) {
      return plane.military ? 'yes' : 'no';
    },
    align: 'right',
  };
  cols.wd = {
    text: 'Wind D.',
    sort: function () {
      sortBy('wd', compareNumeric, function (x) {
        return x.wd;
      });
    },
    value: function (plane) {
      return plane.wd != null ? plane.wd + '°' : '';
    },
    align: 'right',
  };
  cols.ws = {
    text: 'Wind S.',
    sort: function () {
      sortBy('ws', compareNumeric, function (x) {
        return x.ws;
      });
    },
    value: function (plane) {
      return format_speed_brief(plane.ws, DisplayUnits);
    },
    align: 'right',
    header: function () {
      return 'Wind' + NBSP + '(' + get_unit_label('speed', DisplayUnits) + ')';
    },
  };
  ...
  for (let i in colsEntries) {
    ...
    if (!value.td) value.td = value.align ? '<td style="text-align: ' + value.align + '">' : '<td>';
  }
  ...
  planeMan.setColumnVis = function (col, visible) {
    ...
    if (!initializing) planeMan.redraw();
  };
  ...
  planeMan.refresh = function () {
    ...
    for (let i = 0; i < g.planesOrdered.length; ++i) {
      ...
      if (tableInView) {
        if (plane.visible) TrackedAircraft++;
        ...
        if (plane.visible) {
          TrackedAircraft++;
          pList.push(plane);
          if (plane.position != null) TrackedAircraftPositions++;
        }
      }
    }
    ...
    for (let i in pList) {
      ...
      if (now - plane.refreshTR > 5 || plane.selected != plane.selectCache) {
        ...
        if (plane.selected) colors = tableColors.selected;

        if (plane.dataSource && plane.dataSource in colors) bgColor = colors[plane.dataSource];
  ...
  function resortTable(pList) {
    if (!planeMan.sortExtract) return;
    if (globeIndex) {
    ...
    // presort by dataSource
      ...
      pList.sort(function (x, y) {
        ...
        if (a == b) return x._sort_pos - y._sort_pos;
