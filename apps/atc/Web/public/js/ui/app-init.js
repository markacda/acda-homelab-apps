...
function initialize() {
  ...
  jQuery.when(configureReceiver, heatmapDefer).done(function () {
    if (receiverJson) {
      if (receiverJson.trace_hist_only) trace_hist_only = true;
      if (receiverJson.json_trace_interval < 2) traces_high_res = true;
...
function initPage() {
  if (globeIndex) {
    function setGlobeTableLimit() {
      ...
      if (toggles['allTableLines'] && toggles['allTableLines'].state) globeTableLimit = 1e9;
      if (onMobile) globeTableLimit /= 2;
    }
    new Toggle({
      key: 'moreTableLines1',
...
function earlyInitPage() {
  ...
  if (usp.has('halloween')) halloween = true;

  if (usp.has('outlineWidth')) {
    let tmp = parseInt(usp.get('outlineWidth'));
    if (!isNaN(tmp)) outlineWidth = tmp;
  }
  ...
  if (usp.has('mobile')) onMobile = true;
  if (usp.has('desktop')) onMobile = false;

  if (usp.has('hideSidebar')) loStore['sidebar_visible'] = 'false';
  ...
  if (usp.has('tempTrails')) {
    ...
    if (tmp > 0) tempTrailsTimeout = tmp;
  }
  ...
  if (usp.has('mapDim')) {
    ...
    if (!isNaN(dim)) mapDimPercentage = dim;
  } else if (heatmap) {
    mapDimPercentage = 0.6;
    MapDim = true;
  }

  if (usp.has('noRegOnly')) noRegOnly = true;

  if (usp.has('nogpsOnly') || usp.has('badgps')) nogpsOnly = true;

  if (usp.has('mapContrast')) {
    let contrast = parseFloat(usp.get('mapContrast'));
    if (!isNaN(contrast)) mapContrastPercentage = contrast;
  }
  ...
  if (loStore['speedVectorMinutes'] != null) speedVectorMinutes = parseInt(loStore['speedVectorMinutes']) || 2;
  ...
  if (usp.has('hideButtons')) hideButtons = true;
  ...
  if (usp.has('overlays')) enableOverlays = usp.get('overlays').split(',');
  ...
  if (loStore['debug'] == 'true') debug = true;
  if (loStore['debugPosFilter'] == 'true') debugPosFilter = true;
  ...
  if (loStore['sidebar_width'] != null) jQuery('#sidebar_container').width(loStore['sidebar_width']);
  else jQuery('#sidebar_container').width('25%');

  if (jQuery('#sidebar_container').width() > jQuery(window).innerWidth() * 0.8) jQuery('#sidebar_container').width('30%');
  ...
  jQuery('#trace_back_1d').click(function () {
    shiftTrace(-1);
  });
  jQuery('#trace_jump_1d').click(function () {
    shiftTrace(1);
  });
  ...
  jQuery('#leg_prev').click(function () {
    legShift(-1);
  });
  jQuery('#leg_next').click(function () {
    legShift(1);
  });
  ...
  jQuery('#tStop').on('click', function () {
    traceOpts.replaySpeed = 0;
    gotoTime(traceOpts.showTime);
  });
  jQuery('#t1x').on('click', function () {
    replaySpeedChange(1);
  });
  jQuery('#t5x').on('click', function () {
    replaySpeedChange(5);
  });
  jQuery('#t10x').on('click', function () {
    replaySpeedChange(10);
  });
  jQuery('#t20x').on('click', function () {
    replaySpeedChange(20);
  });
  jQuery('#t40x').on('click', function () {
    replaySpeedChange(40);
  });
  ...
  new Toggle({
    ...
    init: true,
    setState: function (state) {
      if (state) monochromeMarkers = null;
      else monochromeMarkers = '#EEEEEE';
  ...
  new Toggle({
    ...
    init: true,
    setState: function (state) {
      if (state) monochromeTracks = null;
      else monochromeTracks = '#000000';
