...
function toggleExtendedLabels(options) {
  if (isNaN(g.extendedLabels)) g.extendedLabels = 0;
...
function toggleMultiSelect(newState) {
  ...
  if (newState == 'on') multiSelect = true;
  if (newState == 'off') multiSelect = false;

  if (!multiSelect) {
    if (!SelectedPlane) toggleIsolation('off');
    if (prevState != multiSelect) deselectAllPlanes('keepMain');
  }
...
function onJump(e) {
  ...
  if (airport) {
    if (!g.airport_cache) {
      jQuery.getJSON('api/globe-airplanes-live/' + databaseFolder + '/airport-coords.js').done(function (data) {
...
function onSearch(e) {
  ...
  let results = [];
  if (searchTerm) results = findPlanes(searchTerm, 'byIcao', 'byCallsign', 'byReg', 'byType', true);
...
function updateAltFilter() {
  ...
  if (minAltitude < -1e6 || minAltitude > 1e6 || isNaN(minAltitude)) minAltitude = -1e6;
  else enabled = true;
  if (maxAltitude < -1e6 || maxAltitude > 1e6 || isNaN(maxAltitude)) maxAltitude = 1e6;
  else enabled = true;
...
Filter.prototype.init = function () {
  ...
  row.innerHTML =
    `<td><form id="${this.id}">` +
    '<div class="infoBlockTitleText">Filter by ' +
    this.name +
    ':</div>' +
  ...
  jQuery(this.sid + '_reset').click((e) => {
    return this.reset(e);
  });
};
...
function getPhotoLink(ac) {
  if (jetphotoLinks) {
    if (ac.registration == null || ac.registration == '') return '';
    return (
      '<a class="link" target="_blank" href="https://www.jetphotos.com/photo/keyword/' +
      ac.registration.replace(/[^0-9a-z]/gi, '') +
      '" rel="noreferrer">Jetphotos</a>'
    );
  } else if (flightawareLinks) {
    if (ac.registration == null || ac.registration == '') return '';
    return (
      '<a class="link" target="_blank" href="https://flightaware.com/photos/aircraft/' +
      ac.registration.replace(/[^0-9a-z]/gi, '') +
      '" rel="noreferrer">FA Photos</a>'
    );
  } else if (showPictures) {
    return (
      '<a class="link" target="_blank" href="https://www.planespotters.net/hex/' +
      ac.icao.toUpperCase() +
      '" rel="noreferrer">View on g.planespotters</a>'
    );
  }
}
