...
function startPage() {
  ...
  if (pTracks) setTimeout(TAR.planeMan.refresh, 10000);
...
(function (global, jQuery, TAR) {
  ...
  utils.make_geodesic_circle = function (center, radius, points) {
    ...
    for (let i = 0; i <= points; ++i) {
      ...
      let lat2 = Math.asin(Math.sin(lat1) * Math.cos(angularDistance) + Math.cos(lat1) * Math.sin(angularDistance) * Math.cos(bearing));
      let lon2 =
        lon1 +
      ...
      if (!geom) geom = new ol.geom.LineString([[lon2, lat2]]);
