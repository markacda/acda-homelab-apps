function toggleShowTrace() {
  ...
  if (showTrace) {
    ...
    if (!showTraceWasIsolation) toggleIsolation('off');
...
function legShift(offset, plane) {
  ...
  if (offset != 0) traceOpts.showTime = null;
  ...
  if (legSel < -1) legSel = count;
  if (legSel > count) legSel = -1;
  ...
  for (let i = legStart + 1; i < trace.length; i++) {
    let timestamp = trace[i][0];
    if (traceOpts.endStamp != null && timestamp > traceOpts.endStamp) break;
    if (trace[i][6] & 2) {
      if (count == legSel - 1) legStart = i;
      if (count == legSel) legEnd = i; // exclusive
      count++;
    }
  }
...
function pollPositionInterval() {
  ...
  timers.pollPosition = window.setInterval(function () {
    ...
    if (new Date().getTime() - lastCallLocationChange < pollPositionSeconds * 0.85 * 1000) return;
...
function geoFindMe() {
  ...
  function success(position) {
...

    {
      ...
      geoFindInterval = window.setInterval(
        function () {
          if (tabHidden) return;
          const geoposOptions = {
            enableHighAccuracy: false,
...
function drawOutlineJson() {
  let request = jQuery.ajax({ url: actualOutline.url, cache: false, timeout: actualOutline.refresh, dataType: 'json' });
  request.done(function (data) {
    ...
    if (!points[0] || !points[0].length) return;
...
function checkFollow() {
  ...
  if (Math.abs(center[0] - proj[0]) > 1 || Math.abs(center[1] - proj[1]) > 1) {
    toggleFollow(false);
    return false;
  }
  return true;
...
function everySecond() {
  if (traceRate > 0) traceRate = traceRate * 0.985 - 1;
  updateIconCache();
}
...
function getTrace(newPlane, hex, options) {
  if (options.list) {
    ...
    if (!newPlane) {
      if (options.onDrain) options.onDrain();
      return;
    }
    hex = newPlane.icao;
  }
  ...
  if (showTrace && trace_hist_only) {
    URL2 =
      'api/globe-airplanes-live/globe_history/' +
      zDateString(refDate).replace(/-/g, '/') +
      '/traces/' +
      hex.slice(-2) +
      '/trace_full_' +
      hex +
  ...
  if (showTrace) {
    ...
    if (traceOpts.startHours == null || traceOpts.startHours < 0) traceOpts.startStamp = traceDate.getTime() / 1000;
    else traceOpts.startStamp = traceDate.getTime() / 1000 + traceOpts.startHours * 3600 + traceOpts.startMinutes * 60 + traceOpts.startSeconds;

    if (traceOpts.endHours == null || traceOpts.endHours >= 24) traceOpts.endStamp = traceDate.getTime() / 1000 + 24 * 3600;
    else traceOpts.endStamp = traceDate.getTime() / 1000 + traceOpts.endHours * 3600 + traceOpts.endMinutes * 60 + traceOpts.endSeconds;
  }
  ...
  if (URL1 && !options.onlyFull) {
    jQuery
      .ajax({ url: `${URL1}`, dataType: 'json', options: options })
      .done(function (data) {
        ...
        if (!showTrace) {
          plane.processTrace();
          if (options.follow) toggleFollow(true);
        }
        ...
        if (options.onlyRecent && options.list) {
          ...
          // catches up.
          if (atcStyle) newPlane.reapTrail();
  ...
  if (options.onlyRecent) return newPlane;

  jQuery
    .ajax({ url: `${URL2}`, dataType: 'json', options: options })
    .done(function (data) {
      const options = this.options;
      const plane = g.planes[options.plane];
    ...
    .fail(function () {
      const options = this.options;
      const plane = g.planes[options.plane];
      if (showTrace) legShift(0, plane);
      else plane.processTrace();
...
function fetchVisibleTrails() {
  if (!atcStyle || showTrace || replay) return;
  ...
  for (let i = 0; i < g.planesOrdered.length; i++) {
    const plane = g.planesOrdered[i];
    if (plane.visible && plane.position != null && !plane.recentTraceRequested) eligible.push(plane);
  }
