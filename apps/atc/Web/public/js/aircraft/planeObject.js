...
function PlaneObject(icao) {
  ...
  this.baseScale = 1;
  this.speedVectorLine = null; // Speed vector line feature
...
PlaneObject.prototype.checkLayers = function () {
  ...
  if ((showTrace || trackLabels) && !this.trail_labels) this.createLabels();
};
...
PlaneObject.prototype.logSel = function (loggable) {
  if (debugTracks && this.selected && !SelectedAllPlanes && !globeIndex) console.log(loggable);
  return;
};
...
PlaneObject.prototype.isFiltered = function () {
  ...
  if (!filterTracks && altFiltered(this.altitude)) return true;
  ...
  if (PlaneFilter.groundVehicles == 'filtered') {
    if (typeof this.category === 'string' && this.category.startsWith('C')) return true;
    if (this.altitude == 'ground' && (this.addrtype == 'adsb_icao_nt' || this.addrtype == 'tisb_other')) return true;
    if (this.squawk == 7777) return true;
  }

  // filter out blocked MLAT flights
  if (PlaneFilter.blockedMLAT == 'filtered') {
    if (typeof this.icao === 'string' && this.icao.startsWith('~')) return true;
  }

  if (this.sitedist && this.sitedist > filterMaxRange) return true;

  return false;
};

function altFiltered(altitude) {
  if (PlaneFilter.minAltitude == null || PlaneFilter.maxAltitude == null) return false;
  if (altitude == null) {
    return true;
  }
...
PlaneObject.prototype.updateTrack = function (now, last, serverTrack, stale) {
  ...
  if (this.prev_position && this.position[0] == this.prev_position[0] && this.position[1] == this.prev_position[1] && !serverTrack) return false;
  if (this.bad_position && this.position[0] == this.bad_position[0] && this.position[1] == this.bad_position[1]) return false;
  ...
  let is_leg = false;
  if (this.leg_ts == now) is_leg = 'end';
  if (this.leg_ts == last) is_leg = 'start';

  if (this.track_linesegs.length == 0) {
    // Brand new track
    //console.log(this.icao + " new track");
    if (this.leg_ts == now) is_leg = 'start';
    let newseg = {
  ...
  if (pFilter) {
    ...
    filterSpeed =
      this.speed != null && this.prev_speed != null
        ? (positionFilterGsFactor * (Math.max(this.speed, this.prev_speed) + 10 + (this.dataSource == 'mlat') * 100)) / 666
        : filterSpeed;
  }
    ...
    if (debugPosFilter) {
      console.log(
        this.icao +
          ...
          ' (' +
          this.dataSource +
          '): Implausible position filtered: ' +
          this.bad_position[0] +
          ', ' +
          this.bad_position[1] +
          ' (kt/Mach ' +
          (derivedMach * 666).toFixed(0) +
          ' > ' +
          (filterSpeed * 666).toFixed(0) +
          ' / ' +
          derivedMach.toFixed(2) +
          ' > ' +
          filterSpeed.toFixed(2) +
          ') (' +
          (this.position_time - this.prev_time + 0.2).toFixed(1) +
          's)'
      );
    }
  ...
  if ((this.position[0] < -90 && this.prev_position[0] > 90) || (this.position[0] > 90 && this.prev_position[0] < -90)) {
    lastseg.fixed.appendCoordinate(projPrev);
  ...
  if (g.refreshHistory || !loadFinished || serverTrack) time_difference = this.position_time - this.prev_time - Math.min(60, now - last);
  ...
  if (pTracks && !serverTrack) {
    stale = false;
    stale_timeout = 120;
    if (this.dataSource == 'adsc') stale_timeout = jaeroTimeout;
  }

  const modeS = this.prev_dataSource == 'modeS';

  if (replay) stale_timeout = 2 * replay.ival + 1;
  ...
  if (
    ...
    debugAll ||
    serverTrack ||
    (!globeIndex && (track_change > 5 || Math.abs(this.prev_speed - lastseg.speed) > 10 || Math.abs(this.prev_alt - lastseg.alt_real) > 200))
    ...
    if (verboseUpdateTrack) {
      this.logSel(
        'sec_elapsed: ' +
          since_update.toFixed(1) +
          ' alt_change: ' +
          alt_change.toFixed(0) +
          ' derived_speed(kt/Mach): ' +
          ((distance_traveled / since_update) * 1.94384).toFixed(0) +
          ' / ' +
          (distance_traveled / since_update / 343).toFixed(1) +
          ' dist:' +
          distance_traveled.toFixed(0)
      );
    }
  ...
  if (
    ...
    this.logSel(
      'sec_elapsed: ' +
        since_update.toFixed(1) +
        ' ' +
        (on_ground ? 'ground' : 'air') +
        ' dist:' +
        distance_traveled.toFixed(0) +
        ' track_change: ' +
        track_change.toFixed(1) +
        ' derived_speed(kt/Mach): ' +
        ((distance_traveled / since_update) * 1.94384).toFixed(0) +
        ' / ' +
        (distance_traveled / since_update / 343).toFixed(1)
...
PlaneObject.prototype.getDataSourceNumber = function () {
  if (this.dataSource == 'modeS') return 5;
  if (this.dataSource == 'adsc') return 6;
  if (this.dataSource == 'mlat') return 3;

  if (this.dataSource == 'uat' || (this.addrtype && this.addrtype.substring(0, 4) == 'adsr')) return 2; // UAT

  if (this.dataSource == 'tisb') return 4; // TIS-B
  if (this.dataSource == 'adsb') return 1;

  if (this.dataSource == 'other' || this.dataSource == 'ais') return 7;
...
PlaneObject.prototype.getMarkerColor = function (options) {
  ...
  if (this.category == 'C3' || this.icaoType == 'TWR' || (this.icaoType == null && this.squawk == 7777)) alt = 'ground';
  ...
  if ((this.dataSource == 'adsc' && this.seen_pos > 20 * 60) || (!globeIndex && this.dataSource != 'adsc' && this.seen_pos > 15)) {
...
PlaneObject.prototype.setMarkerRgb = function () {
  ...
  if (this.shape && this.shape.svg) rgb = [255, 255, 255];
...
PlaneObject.prototype.updateIcon = function () {
  ...
  if (
    ...
    let callsign = '';
    if (this.flight && this.flight.trim() && !(this.dataSource == 'ais' && !g.extendedLabels)) callsign = this.flight.trim();
    else if (this.registration) callsign = 'reg: ' + this.registration;
    else callsign = 'hex: ' + this.icao;
    if (this.dataSource == 'ais' && this.routeString) {
  ...
  if (this.styleKey != styleKey || !this.marker.getStyle()) {
    ...
    if (labelText) {
      ...
      if (!webgl) styles.unshift(new ol.style.Style({ image: this.markerIcon, zIndex: this.zIndex }));
      this.marker.setStyle(styles);
    } else {
      this.markerStyle = new ol.style.Style(webgl ? { zIndex: this.zIndex } : { image: this.markerIcon, zIndex: this.zIndex });
...
PlaneObject.prototype.processTrace = function () {
  ...
  if (replay && !this.fullTrace) return;
  ...
  if (showTrace || replay) this.setNull();
  ...
  if (this.fullTrace && this.recentTrace && this.fullTrace.length > 0 && this.recentTrace.length > 0) {
    ...
    let start2 = t2[0][0];
    if (end1 < start2) console.log('Insufficient recent trace overlap!');
  }
  ...
  if (trace && trace.length > 0) {
    ...
    if (legStart != null) start = legStart;
    if (legEnd != null) end = legEnd;
    ...
    for (let i = start; i < end; i++) {
      ...
      if (firstPos == null) firstPos = this.position;
  ...
  if (traceOpts.follow) toggleFollow(true);
  if (traceOpts.showTime) {
...
PlaneObject.prototype.updateData = function (now, last, data, init) {
  ...
  if (newAlt == null || (newAlt == this.bad_alt && this.seen_pos > 5)) {
    // do nothing
  } else if (!altitudeFilter || this.altitude == null || newAlt == 'ground' || this.altitude == 'ground' || (seen_pos != null && seen_pos < 2)) {
    this.altitude = newAlt;
    this.altitudeTime = now;
  } else if (
    this.alt_reliable > 0 &&
    this.altBad(newAlt, this.altitude, this.altitudeTime, data) &&
    ...
    if (debugPosFilter) {
      console.log(
        (now % 1000).toFixed(0) +
          ...
          newAlt +
          ' elapsed: ' +
          (now - this.altitudeTime).toFixed(0)
      );
      jumpTo = this.icao;
    }
  } else {
  ...
  if (data.baro_rate != null) this.baro_rate = data.baro_rate;
  else if (data.vert_rate != null) this.baro_rate = data.vert_rate;
  else this.baro_rate = null;
  ...
  this.rc = data.rc;
  if (!replay || data.squawk != null) this.squawk = data.squawk == null ? null : `${data.squawk}`;
  this.wd = data.wd;
  this.ws = data.ws;
  this.oat = data.oat;
  this.tat = data.tat;
  ...
  if (data.true_heading != null) this.true_heading = data.true_heading;
  else this.true_heading = null;

  if (type != null) this.addrtype = type;
  else this.addrtype = null;
  ...
  if (replay) {
    ...
    if (this.true_heading != null) this.rotation = this.true_heading;
    else if (this.mag_heading != null) this.rotation = this.mag_heading;
    else if (data.calc_track) this.rotation = data.calc_track;
  ...
  if (this.position != null && this.track != null && this.gs != null) {
    ...
    const gsMetersPerSecond = this.gs * 0.514444;

    // Calculate distance traveled in projection time
    const distanceMeters = gsMetersPerSecond * projectionSeconds;

    // Convert track from degrees to radians
    const trackRadians = this.track * (Math.PI / 180);

    // Convert current latitude to radians for longitude calculation
    const latRadians = currentLat * (Math.PI / 180);

    // Calculate projected position
    // Latitude change: distance * cos(bearing) / meters per degree
    const latChange = (distanceMeters * Math.cos(trackRadians)) / 111111;

    // Longitude change: distance * sin(bearing) / (meters per degree * cos(latitude))
    const lonChange = (distanceMeters * Math.sin(trackRadians)) / (111111 * Math.cos(latRadians));

    this.speedVectorLat = currentLat + latChange;
    this.speedVectorLon = currentLon + lonChange;
  } else {
    // Reset if we don't have required data
    this.speedVectorLat = null;
    this.speedVectorLon = null;
  }
...
PlaneObject.prototype.updateFeatures = function (redraw) {
  if (this.visible) {
    if (this.drawLine || redraw || this.lastVisible != this.visible) this.updateLines();
...
PlaneObject.prototype.clearMarker = function () {
  ...
  // Clear speed vector line
  if (this.speedVectorLine && this.speedVectorLine.visible) {
    PlaneIconFeatures.removeFeature(this.speedVectorLine);
    this.speedVectorLine.visible = false;
  }
  delete this.speedVectorLine;
...
PlaneObject.prototype.updateMarker = function (moved) {
  if (!this.visible || this.position == null || (pTracks && (SelectedAllPlanes || !this.selected))) {
    if (this.markerDrawn) this.clearMarker();
    return;
  }
  ...
  if (this.speed > 120) {
    if (icaoType == 'V22') icaoType = 'V22F';
    if (icaoType == 'B609') icaoType = 'B609F';
  }
  if (icaoType == null && this.squawk == 7777) icaoType = 'TWR';
  let baseMarkerKey = this.category + '_' + this.typeDescription + '_' + this.wtc + '_' + icaoType + '_' + (this.altitude == 'ground') + eastbound;
  ...
  // Update speed vector line
  if (atcStyle && this.showSpeedVector && this.speedVectorLat != null && this.speedVectorLon != null && this.position != null) {
    // Create line geometry from current position to projected position
    const lineCoords = [this.olPoint.getCoordinates(), ol.proj.fromLonLat([this.speedVectorLon, this.speedVectorLat])];
    const lineGeometry = new ol.geom.LineString(lineCoords);

    // Get marker color to use for the line
    const markerColor = hslToRgb(this.getMarkerColor());

    // Create or update the line feature
    if (!this.speedVectorLine) {
      this.speedVectorLine = new ol.Feature(lineGeometry);
      this.speedVectorLine.hex = `${this.icao}_vector`;
    } else {
      this.speedVectorLine.setGeometry(lineGeometry);
    }

    // Set line style with same color as marker
    const lineStyle = new ol.style.Style({
      stroke: new ol.style.Stroke({
        color: markerColor,
        width: 2,
        // lineDash: [5, 5] // Dashed line to differentiate from tracks
      }),
    });
    this.speedVectorLine.setStyle(lineStyle);

    // Add to map if not visible
    if (!this.speedVectorLine.visible) {
      this.speedVectorLine.visible = true;
      PlaneIconFeatures.addFeature(this.speedVectorLine);
    }
  } else {
    // Remove speed vector line if it exists but shouldn't be shown
    if (this.speedVectorLine && this.speedVectorLine.visible) {
      PlaneIconFeatures.removeFeature(this.speedVectorLine);
      this.speedVectorLine.visible = false;
    }
...
function altitudeLines(segment) {
  ...
  if (segment.estimated) colorArr = [colorArr[0], colorArr[1], colorArr[2] * 0.8];
  //let color = 'hsl(' + colorArr[0].toFixed(0) + ', ' + colorArr[1].toFixed(0) + '%, ' + colorArr[2].toFixed(0) + '%)';

  let color = hslToRgb(colorArr);

  if (monochromeTracks) color = monochromeTracks;
  ...
  if (lineStyleCache[lineKey]) return lineStyleCache[lineKey];
  ...
  if (noVanish) multiplier *= segment.estimated ? 0.3 : 0.6;
...
PlaneObject.prototype.updateLines = function () {
  ...
  if (trackLabels || showTrace) {
    if (!this.layer_labels.getVisible()) this.layer_labels.setVisible(true);
  ...
  for (let i = this.track_linesegs.length - 1; i >= 0; i--) {
    ...
    if (seg.feature && (!trackLabels || seg.label)) break;
    ...
    if (seg.label) {
      ...
      seg.label = true;
    } else if (trackLabels || ((i == 0 || i == this.track_linesegs.length - 1 || seg.leg) && showTrace && g.enableLabels)) {
      ...
      if (showTrace && !trackLabels) text = timestamp1 + timestamp2;
  ...
  if (trail_add.length > 0) this.trail_features.addFeatures(trail_add);
  if (this.trail_labels && label_add.length > 0) this.trail_labels.addFeatures(label_add);
};
...
PlaneObject.prototype.removeTrail = function () {
  if (this.trail_features) this.trail_features.clear();
  if (this.trail_labels) this.trail_labels.clear();
...
PlaneObject.prototype.makeTR = function (trTemplate) {
  ...
  // Right-click to toggle speed vector
  this.contextmenuListener = (evt) => {
    evt.preventDefault(); // Prevent default browser context menu

    // Toggle speed vector
    this.showSpeedVector = !this.showSpeedVector;

    // Update the marker to show/hide the vector
    this.updateMarker();
  };

  this.tr.addEventListener('contextmenu', this.contextmenuListener);
};
PlaneObject.prototype.destroyTR = function (trTemplate) {
  ...
  this.tr.removeEventListener('contextmenu', this.contextmenuListener);

  delete this.clickListener;
  delete this.dblclickListener;
  delete this.contextmenuListener;
...
function hslToRgb(arr, opacity) {
  ...
  if (opacity == 'array') return [Math.round(r * 255), Math.round(g * 255), Math.round(b * 255)];

  if (opacity != null) return 'rgba(' + Math.round(r * 255) + ', ' + Math.round(g * 255) + ', ' + Math.round(b * 255) + ', ' + opacity + ')';
  else return 'rgb(' + Math.round(r * 255) + ', ' + Math.round(g * 255) + ', ' + Math.round(b * 255) + ')';
}

PlaneObject.prototype.altBad = function (newAlt, oldAlt, oldTime, data) {
  let max_fpm = 12000;
  if (data.geom_rate != null) max_fpm = 1.3 * Math.abs(data.goem_rate) + 5000;
  else if (data.baro_rate != null) max_fpm = 1.3 * Math.abs(data.baro_rate) + 5000;
...
PlaneObject.prototype.milRange = function () {
  ...
  for (let i in milRanges) {
    const r = milRanges[i];
    if (this.numHex >= r[0] && this.numHex <= r[1]) return true;
  }
  return false;
};
...
PlaneObject.prototype.updateTraceData = function (state, _now) {
  ...
  if (track) this.rotation = track;
  else this.request_rotation_from_track = true;
  ...
  if (ias !== undefined) this.ias = ias;
  if (type !== undefined) this.addrtype = type == null ? null : `${type}`;

  if (rId !== undefined) this.rId = rId;
...
PlaneObject.prototype.cross180 = function (on_ground, is_leg) {
  ...
  while ((tmp = out.shift()) != null) {
    if (sign1 == Math.sign(tmp[0])) seg1.push(tmp);
    else seg2.push(tmp);
  }
  //console.log([...seg1]);
  //console.log([...seg2]);
  ...
  for (let i in seg1) seg1[i] = ol.proj.fromLonLat(seg1[i]);
  for (let i in seg2) seg2[i] = ol.proj.fromLonLat(seg2[i]);
...
PlaneObject.prototype.checkVisible = function () {
  ...
  if (this.dataSource == 'mlat') {
    timeout = seenTimeoutMlat;
  } else if (this.dataSource == 'adsc') {
    timeout = jaeroTimeout;
  } else if (this.dataSource == 'ais') {
    timeout = aisTimeout;
  }
  ...
  const res =
    (!globeIndex || icaoFilter || this.inView || this.selected || SelectedAllPlanes) &&
...
PlaneObject.prototype.setTypeData = function () {
  if (g.type_cache == null || !this.icaoType || this.icaoType == this.icaoTypeCache) return;
  this.updateMarker();
  ...
  if (!(typeCode in g.type_cache)) return;

  let typeData = g.type_cache[typeCode];
  const typeLong = typeData[0];
  const desc = typeData[1];
  const wtc = typeData[2];
  if (desc != null) this.typeDescription = `${desc}`;
  if (wtc != null) this.wtc = `${wtc}`;
  if (this.typeLong == null && typeLong != null) this.typeLong = `${typeLong}`;
};
...
PlaneObject.prototype.updateAlt = function (t) {
  ...
  if (this.fakeHex) this.zIndex -= 100000;
};
...
PlaneObject.prototype.routeCheck = function () {
  ...
  if (!this.flight || !this.name || this.name == 'empty callsign' || this.name == 'no callsign' || this.registration == this.name) {
    this.routeString = '';
    this.routeVerbose = '';
    return;
  }
