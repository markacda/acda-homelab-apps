function processAircraft(ac, init, uat) {
  ...
  if (icaoFilter && !icaoFilter.includes(hex)) return;

  if (icaoBlacklist && icaoBlacklist.includes(hex)) return;
  ...
  if (!uat) {
    if (
      !plane.uat || // plane isn't uat, new data isn't uat, accept immediately
    ...
    if (
      plane.uat || // plane is uat, new data is uat, accept immediately
      (prefer978 > 0 && newPos < prefer978) ||
...
function processReceiverUpdate(data, init) {
  ...
  if (globeIndex) {
    if ((showGrid || loStore['globeGrid'] == 'true') && globeIndexNow[data.globeIndex] == null) drawTileBorder(data);
...
function fetchData(options) {
  ...
  if (!options.force) {
    if (currentTime - lastFetch <= refreshMs || pendingFetches > 0 || OLMap.getView().getInteracting() || OLMap.getView().getAnimating()) {
      return;
    }
  }
  ...
  if (enable_uat) {
    FetchPendingUAT = jQuery.ajax({ url: 'chunks/978.json', dataType: 'json' });
  ...
  for (let i in ac_url) {
    ...
    req.done(fetchDone).fail(fetchFail);
  }
