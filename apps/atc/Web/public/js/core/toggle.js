...
Toggle.prototype.init = function () {
  ...
  if (this.button) {
    jQuery(this.button).on('click', () => {
      this.toggle();
    });
  }

  if (loStore[this.key] == 'true') this.state = true;
  if (loStore[this.key] == 'false') this.state = false;

  this.toggle(this.state, true);
};

Toggle.prototype.toggle = function (override, init) {
  if (override == true) this.state = true;
  else if (override == false) this.state = false;
  ...
  if (!init) loStore[this.key] = this.state;
};

Toggle.prototype.restore = function () {
  if (this.setState) this.setState(this.state);
};

Toggle.prototype.hideCheckbox = function () {
  if (this.checkbox) jQuery(this.checkbox).parent().hide();
};
