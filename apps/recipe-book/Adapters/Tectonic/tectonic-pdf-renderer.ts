...
export class TectonicPdfRenderer implements DocumentGenerator {
  ...
  async generatePdf(book: Book, recipes: Recipe[]): Promise<string> {
    ...
    try {
      ...
      if (e.code === 'ENOENT') {
        throw new Error("PDF generation needs the 'tectonic' LaTeX engine, which is not installed on this host.")
      }
  ...
  private async loadTemplates(): Promise<Templates> {
    const [book, recipe] = await Promise.all([readFile(join(TEMPLATES_DIR, 'book.tex'), 'utf8'), readFile(join(TEMPLATES_DIR, 'recipe.tex'), 'utf8')])
    return { book, recipe }
  }
