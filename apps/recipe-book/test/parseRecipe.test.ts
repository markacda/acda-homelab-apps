...
test('normalizeAllerhandeUrl rewrites the short recipe URL to canonical', () => {
  assert.equal(normalizeAllerhandeUrl('https://www.ah.nl/r/480288'), 'https://www.ah.nl/allerhande/recept/r-480288')
  // Any recipe number, trailing slash, and no-www host are all handled.
  assert.equal(normalizeAllerhandeUrl('https://ah.nl/r/12/'), 'https://ah.nl/allerhande/recept/r-12')
...
test('parseRecipe extracts a Recipe from a @graph', () => {
  ...
  assert.deepEqual(parsed.ingredients, ['250 g pappardelle', '2 el olijfolie', '300 g kogelbiefstuk'])
