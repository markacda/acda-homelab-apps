...
export function stackedBarChart(data: ChartDatum[], series: ChartSeries[]): HTMLElement {
  ...
  for (const frac of [0, 0.5, 1]) {
    ...
    root.append(
      ...
      svg(
        "text",
        { x: PAD.left - 6, y: y + 3, class: "chart-axis", "text-anchor": "end" },
        String(v),
      ),
    );
  }
  ...
  for (const i of xLabelIdx) {
    ...
    root.append(
      svg(
        "text",
        { x, y: HEIGHT - 8, class: "chart-axis", "text-anchor": "middle" },
        shortBucket(data[i].bucket),
      ),
    );
  }
  ...
  const legend = el(
    ...
    ...series.map((s) =>
      el(
        ...
        el("span", { class: "chart-swatch", style: `background: var(${s.varName})` }),
        s.label,
      ),
    ),
  );
