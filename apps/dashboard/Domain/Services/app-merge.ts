 ...
 */
export function mergeApps(discovered: AppEntry[], config: Pick<Config, 'apps' | 'overrides'>): AppEntry[] {
  const merged: AppEntry[] = [];

  for (const app of discovered) {
    const override = (app.containerName ? config.overrides[app.containerName] : undefined) || (app.name ? config.overrides[app.name] : undefined);
